...
bool CatalogManager :: CreateIndex( string name, string table_name, string attr_name  ){
    ...
    t.attr_.has_index[index] = true;
    t.attr_.index_name[index] = name;
    return true;
}
bool CatalogManager :: HasIndex( string name ){
    map<string, string > :: iterator SSItor = INDEXSET.find(name);
    return SSItor != INDEXSET.end(); 
}
bool CatalogManager :: HasTable( string name ){
...
bool CatalogManager :: HasAttribute(string table_name, string attr_name){
    Table & t = GetTable( table_name );
    return t.HasAttribute( attr_name ) != -1;
}
